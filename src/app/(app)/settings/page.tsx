...
/** Human-readable outcomes of the WooCommerce authorization redirect. */
const AUTH_MESSAGES: Record<string, { title: string; detail: string; tone: "good" | "bad" }> = {
  connected: {
  ...
  not_reachable: {
    ...
    detail:
      "WooCommerce delivers the key by POSTing to this app from your store's server, so a localhost or private-network address can never receive it. Expose this app on a public HTTPS URL — a tunnel such as `cloudflared tunnel --url https://localhost:3000` or `ngrok http https://localhost:3000` is the quickest way locally — then set APP_URL to that address and try again.",
    tone: "bad",
  },
  bad_app_url: {
