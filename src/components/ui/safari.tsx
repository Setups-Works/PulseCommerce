...
export interface SafariProps extends HTMLAttributes<HTMLDivElement> {
  ...
  mode?: SafariMode
  /**
   * Live DOM to render into the browser's screen area, as an alternative to
   * `imageSrc` or `videoSrc`.
   *
   * Added to the registry component because this product's screenshots are
   * built from its own components rather than captured — see the note at the
   * top of components/marketing/sections.tsx. A real element also stays sharp
   * at every viewport width and keeps its text selectable and translatable,
   * which a scaled PNG of a dashboard does not.
   */
